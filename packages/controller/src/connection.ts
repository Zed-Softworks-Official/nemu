...
import { getClientToken, getRememberedControllerId, setRememberedBaseUrl, setRememberedControllerId } from './storage'
...
export class ControllerConnection {
    ...
    private startReprobeLoop(): void {
        ...
        this.reprobeTimer = setInterval(() => {
            if (this.status.mode === 'relay' || this.status.mode === 'offline') {
